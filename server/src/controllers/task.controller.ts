...
export class TaskController {
    ...
    delete(req: Request, res: Response, next: NextFunction) {
        return TaskService.delete(req.params.id)
            .then((task: Task) => {
     ...
     * @apiGroup Task
     * 
     * @apiParam {Boolean}  [isComplete=false]  Boolean to query for complete or incomplete Tasks with default false or incomplete.
     * @apiParam {String}   [filter=""]         Filter to query Tasks on with default "" (can filter on any Task property other than isComplete).
    ...
    list(req: Request, res: Response, next: NextFunction) {
        ...
        return TaskService.list(isComplete, req.query.filter, req.query.sort, req.query.order, pageNumber, pageSize)
            .then( ([tasks, total]) => {
                return res.json({
                    meta: {
                        total: total,
    ...
    update(req: Request, res: Response, next: NextFunction) {
        if(req.body.meta && req.body.meta.transition == 'complete') {
            if(req.params.id == null) {
                return TaskService.completeAll(req.body.data.ids, req.body.data.properties)
                    .then((tasks: Task[]) => {
                        return res.json(tasks);
                    }).then(next, next);
            }
            else {
                return TaskService.complete(req.params.id, req.body.data.properties)
                    .then((task: Task) => {
                        return res.json(task);
                    }).then(next, next);
            }
            ...
            let properties: Partial<Task> = req.body.data ? req.body.data.properties : {};
            ...
            return TaskService.update(req.params.id, properties, updateAll)
                .then((task: Task) => {
                    return res.json(task);
                }).then(next, next);
        }
