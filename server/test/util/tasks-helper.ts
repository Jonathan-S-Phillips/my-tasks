...
export class TasksHelper {
    ...
    constructor(tasks: any[]) {
        ...
        this.tasksDueToday = this.filterTasks(false, today.format('MM/DD/YYYY'));
        this.tasksDueTomorrow = this.filterTasks(false, tomorrow.format('MM/DD/YYYY'));
        ...
        if(filter != null && filter != "") {
            ...
            if(date.isValid()) {
                ...
                tasks = this.tasks.filter((task: any) => {
                    ...
                    return task.name.toLowerCase().indexOf(filter) >= 0 || task.description.toLowerCase().indexOf(filter) >= 0;
                })   
            }
        }
        else {
    ...
    getTasksDue(date: Date, tasks?: any[]) {
        ...
        return tasks.filter((task: any) => {
            if(task.isComplete) {
                ...
                (task.dateCompleted.getDate() == date.getDate() && task.dateCompleted.getMonth() == date.getMonth() && task.dateCompleted.getFullYear() == date.getFullYear());
            }
            else {
                return task.dueDate.getDate() == date.getDate() && task.dueDate.getMonth() == date.getMonth() && task.dueDate.getFullYear() == date.getFullYear();
            }
